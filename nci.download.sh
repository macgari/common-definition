...
curl --request GET --url $UMLS_URL?ticket=$(curl --request POST --url $UTS_URL?username=$NCI_USERNAME&password=$NCI_PASSWORD --header 'Cache-Control: no-cache') --header 'Cache-Control: no-cache'  --silent --output $NCI_FILE